      ...
      <aside className="w-64 bg-white shadow-lg flex flex-col">
        ...
        <nav className="flex-1 px-4 space-y-2">
          {[
            ...
            { label: "User Help", icon: <User size={18} /> },
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="p-6 space-y-6 overflow-y-auto">
          ...
          {activePage === "User Help" && (
            <section className="space-y-6">
              <div className="bg-white rounded-2xl shadow p-6">
                <h2 className="text-lg font-semibold mb-4">👤 Help Dashboard</h2>
                ...
                {contactLoading ? (
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      ...
                      <table className="min-w-full border border-gray-200 rounded-lg">
                        <thead className="bg-gray-100">
                          <tr>
                            ...
                            <th className="px-4 py-2 text-left text-sm font-medium text-gray-600">Joined</th>
                          </tr>
                        </thead>
                        <tbody>
                          {contact.map((c) => (
                            <tr key={c.id} className="border-t">
                              ...
                              <p className="mt-2 text-gray-700">{c.message ?? "No message"}</p>
                              <td className="px-4 py-2 text-sm text-gray-500">
                                {getTs(c) ? new Date(getTs(c) as string).toLocaleDateString() : "—"}
                              </td>
                            </tr>
                          ))}
                        </tbody>
                    ...
                    {/* Right: Latest message (newest row is contacts[0]) */}
                    <div className="bg-white rounded-2xl shadow-md p-6">
                      ...
                      {contact.length === 0 ? (
                        ...
                        <div
                          className={`space-y-4 ${contact.length > 8 ? "max-h-[600px] overflow-y-auto pr-2" : ""
                            }`}
                        >
                          {contact.map((c) => (
                            <div
                              key={c.id}
                              className="p-5 rounded-xl bg-gradient-to-r from-indigo-50 to-purple-50 border border-gray-200 shadow-sm"
                            >
                              {/* Message */}
