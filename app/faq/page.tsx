...
export default function FAQPage() {
  ...
  if (!window.speechSynthesis) {
    return alert("❌ Your browser does not support speech.");
  }
  ...
  const text =
    lang === "hi" && faq.a_hi ? `${faq.q}. ${faq.a_hi}` : `${faq.q}. ${faq.a}`;
  ...
  if (lang === "hi") {
    ...
    utterance.voice =
      voices.find(v => preferredHindiVoices.includes(v.name)) ||
    ...
    // Preferred English voices (sweet female voices are clearer)
    ...
    utterance.voice =
      voices.find(v => preferredEnglishVoices.includes(v.name)) ||
      voices.find(v => v.lang.startsWith("en")) ||
      voices[0];

    // Softer + expressive tone
    utterance.rate = 0.95;
